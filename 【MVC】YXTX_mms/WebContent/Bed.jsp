...
<body>
		...
		<div class="input-user" style=" display: flex;">
			...
			<table lay-filter="demo">
				<thead>
					<tr>
						<th lay-data="{field:'bed_id', width:120, sort: true}">病床I编号</th>
				...
				<c:forEach var="bed" items="${selBedList }" varStatus="item">
					<tbody>
						<tr>
							...
							<td>${ bed.bed_price}</td>
							<td>
							<c:if test="${ bed.bed_state==1}">
								使用中
							</c:if>
							<c:if test="${ bed.bed_state==0}">
								未使用
							</c:if>
							</td>
							...
							<td>
								...
								<a onclick="del(this)" href="DelPatientServlet?patient_id=${bed.bed_id }"
									class="layui-icon">&#xe640;</a>
							</td>
