...
<body>
	<div class="mainList">
		<div class="" style="float: left;">
			<a href="DoctorInfoAddT.do" class="layui-btn layui-btn-primary layui-border-green" style="margin-left: 20px;">添加</a>
		</div>
		...
		<table lay-filter="demo">
			...
			<tbody>
			<c:forEach var="doctor" items="${DoctorInfo}" varStatus="item">
				<tr>
					<td>${doctor.doctorId}</td>
					<td>${doctor.doctorName}</td>
					<td>${doctor.doctorSex}</td>
					<td>${doctor.doctorTitle}</td>
					<td>${doctor.doctorJob}</td>
					<td>${doctor.doctorSubject}</td>
					<td>${doctor.doctorBirthday}</td>
					<td>${doctor.jobTiem}</td>
					<td>
						<a href="DoctorInfoUpdT.do?doctorId=${doctor.doctorId}" class="layui-btn layui-btn-primary layui-border-black">修改</a>
						<a href="DoctorInfoDel.do?doctorId=${doctor.doctorId}" class="layui-btn layui-btn-danger">删除</a>
					</td>
				</tr>
			</c:forEach>
			</tbody>
